...
public class CarDriver : MonoBehaviour
{
    ...
    private float minSpeedToTurn = 2f;
    ...
    private void Update()
    {
        ...
        carRigidbody.velocity = new Vector3(newVelocity.x, carRigidbody.velocity.y, newVelocity.z);

        // Only turn if in movement
        if (carRigidbody.velocity.magnitude > minSpeedToTurn)
        {
            if (speed < 0)
            {
            ...
            carRigidbody.angularVelocity = new Vector3(carRigidbody.angularVelocity.x, turnSpeed * (invertSpeedNormalized * 1f) * Mathf.Deg2Rad, carRigidbody.angularVelocity.z);
