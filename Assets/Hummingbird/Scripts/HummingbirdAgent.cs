...
public class HummingbirdAgent : Agent
{
    ...
    /// <summary>
    /// Called when an action is received from either the playe input or the neural network
    /// 
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        // Get a vector from the beak tip to the nearest flwoer
        ...
        // (+1 means that the beak tip is directly in fron of the flower, -1 means directly behind)
    ...
    private void UpdateNearestFlower()
    {
        foreach (Flower flower in flowerArea.Flowers)
        {
            ...
            {
                // No current nearest flwoer and this flwoer has nectar, so set to this flower
    ...
    private void TriggerEnterOrStay(Collider collider) {
        ...
        if (collider.CompareTag("nectar")) {
            ...
            if (Vector3.Distance(beakTip.position, closestPointToBeakTip) < BeakTipRadius)
            {
                ...
                // If flower is empty, update the nearest flwoer
                if (!flower.HasNectar)
                {
                    UpdateNearestFlower();
                }
